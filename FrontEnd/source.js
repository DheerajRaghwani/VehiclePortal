...
function displayVehicles(data) {
	...
	data.forEach((v) => {
		...
		row.innerHTML = `
            ...
            <td><button class="sourceBtn" data-vehicleno="${
							v.vehicleNo
						}">Village Departure</button></td>
        `;
...
addSourceBtn.addEventListener("click", async () => {
	...
	try {
		...
		console.error(err);
		alert("Error adding source record");
	}
});
...
async function searchPassedVehicles() {
	...
	try {
		const res = await fetch(
			`${apiBaseUrl}/search?vehicleNo=${encodeURIComponent(vehicleNo)}`,
			{
				headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
			}
		);
...
function displayPassedVehicles(data) {
	...
	data.forEach((r) => {
		...
		row.innerHTML = `
            ...
            <td>${r.nodalMobileNo || "-"}</td>
            <td>${
							r.currentDate ? new Date(r.currentDate).toLocaleDateString() : "-"
						}</td>
        `;
