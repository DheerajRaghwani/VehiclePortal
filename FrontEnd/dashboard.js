// API URLs
const vehicleApiUrl = "https://localhost:7005/api/Dashboard/GetByDistrict"; // Vehicle dashboard
const checkpostApiUrl = "https://localhost:7005/api/CheckpostDashboard"; // Checkpost
const foodApiUrl = "https://localhost:7005/api/DistrictDashboard"; // districtsource
...
if (Array.isArray(vehicleData)) {
    ...
    let totalCapacity = vehicleData.reduce((sum, d) => sum + (d.totalCapacityPerDistrict || 0), 0);
    document.getElementById("vehicleOverallCapacity").textContent = totalCapacity.toLocaleString();
...
async function loadCheckpostDashboard() {
    ...
    try {
        ...
        if (Array.isArray(checkpostData)) {
            const totalOverall = checkpostData.reduce((sum, d) => sum + (d.totalPeopleInDistrict || 0), 0);
            document.getElementById("checkpostOverallPeople").textContent = totalOverall.toLocaleString();
...
// ------------------- Food Status Dashboard -------------------
 async function loadFoodDashboard() {
    ...
    // Backend returned plain list
    let totalPeople = foodData.reduce((sum, d) => sum + (d.totalPeopleInDistrict || 0), 0);
    document.getElementById("foodOverallPeople").textContent = totalPeople.toLocaleString();
...
function updateTopCards() {
    ...
    if (vehicleData) {
        document.getElementById("vehicleOverallCapacity").textContent = vehicleData.overallCapacity.toLocaleString();
    }

    // Checkpost Total People
    if (checkpostData) {
        document.getElementById("checkpostTotalPeopleCard").textContent = checkpostData.overallPeople.toLocaleString();
    }

    // Food Total People
    if (foodData) {
        document.getElementById("foodOverallPeople").textContent = foodData.overallPeople.toLocaleString();
    }

    // Total Buses and Total Small Vehicles (from Vehicle Dashboard)
    if (vehicleData && vehicleData.districtData) {
        let totalBuses = 0, totalSmallVehicles = 0;
        vehicleData.districtData.forEach(d => {
            totalBuses += Number(d.numberOfBuses) || 0;
            totalSmallVehicles += Number(d.numberOfSmallVehicles) || 0;
        });
        document.getElementById("totalBusesCard").textContent = totalBuses;
        document.getElementById("totalSmallVehiclesCard").textContent = totalSmallVehicles;
