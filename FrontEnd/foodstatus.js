const apiUrl = "/api/Foodstatus"; // Update with your API URL
...
async function loadDistricts() {
	...
	try {
		const res = await fetch(`${apiUrl}/DistrictDropdown`, {
...
districtDropdown.addEventListener("change", async () => {
	...
	try {
		const res = await fetch(`${apiUrl}/District/${districtName}`, {
			headers: { Authorization: `Bearer ${token}` },
		});
...
async function loadTable(filters = {}) {
	...
	try {
		...
		if (filters.districtName)
			data = data.filter((d) => d.districtName === filters.districtName);
		if (filters.blockName)
			data = data.filter((d) => d.blockName === filters.blockName);
...
async function openEditModal(id) {
	currentEditId = id;
	// Fetch current data
	const res = await fetch(`${apiUrl}/GetById/${id}`, {
		headers: { Authorization: `Bearer ${token}` },
	});
